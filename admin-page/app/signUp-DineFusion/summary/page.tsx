...
export default function SummaryPage() {
    ...
    const total = useMemo(()=>{
        ...
        addOns.forEach(addOnId => {
            ...
            total += add? add.price[period as keyof typeof add.price] : 0
        })
        total += planData? planData.price[period as keyof typeof planData.price] : 0
        return total
        ...
        <div className="rounded-lg bg-alabaster p-4 space-y-4">
            <div className="flex items-center justify-between text-primary">
                <div>
                    ...
                    <Link href="signUp-DineFusion/plan" className="text-sm underline transition cursor-pointer text-muted-dark hover:text-confirm">Change</Link>
                </div>
                <div className='font-semibold' >
                    ${planData?.price[period as keyof typeof planData.price]}/{period==="monthly"?'mo':'yr'}
                </div>
            </div>
                ...
                addOns.map(id => {
                    ...
                    return (
                        <div key={id} className="flex justify-between items-center text-sm">
                            ...
                            <span className="block text-primary">+${addOn?.price[period as keyof typeof addOn.price]}/{period==='monthly'?'mo':'yr'}</span>
                        </div>
                    )
                })
            }
